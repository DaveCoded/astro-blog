...
import FacultyTable from '@components/posts/database-normalization-notes/faculty-table.astro';

<Dropcap>Normalization</Dropcap> is a database design methodology. Proposed by Edgar F. Codd in 1970
- and subsequently developed in the following years with some collaborators - database normalization
gives us a framework for structuring relational (usually) databases. Its aim is to reduce [data redundancy](#data-redundancy),
and to improve [data integrity](#data-integrity). This is done by adhering to the "**normal forms**",
of which there are *technically* 10 (or 11 if you count "Unnormalized form", which I don't). Fortunately,
however, a database is generally considered to be normalized by **following the first three norms**.
